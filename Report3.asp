<Html>
 ...
 <Body Class="Report">
  ...
  <Form Method="POST" Action="Report3.asp?ye=<%=ya%>">
   ...
   <Table Border="1" width="100%">
    ...
    <%mdbor6a.Open mdbo6a%>
    <%mdbo7.CommandText="SELECT ISNULL(ROUND(SUM(ISNULL(GP.DEBET,0))/1000,0),0)-ISNULL(ROUND(SUM(ISNULL(GP.CREDIT,0))/1000,0),0) AS summym FROM dbo.Main m INNER JOIN dbo.glav_project GP ON m.OracleCode = GP.PROJECT WHERE m.Yearr='" & ya & "' AND (konto BETWEEN '18410' AND '18433') AND GP.MES <" & Mid(ya,4,1) & "07 AND (m.IDentIfier = 'C')"%>
    <%mdbor7.Open mdbo7%>
    <tr Class="boldProjGrup">
    ...
    <%Do Until mdborl1.EOF%>
     ...
     <%mdbor6a.Open mdbo6a%>
     <%mdbo7.CommandText="SELECT ISNULL(ROUND(SUM(ISNULL(GP.DEBET,0))/1000,0),0)-ISNULL(ROUND(SUM(ISNULL(GP.CREDIT,0))/1000,0),0) AS summym FROM dbo.Main m INNER JOIN dbo.glav_project GP ON m.OracleCode = GP.PROJECT WHERE m.Yearr='" & ya & "' AND SUBSTRING(m.ProjCode,1,2)='" & MId(mdborl1("PC"),1,2) & "' AND (konto BETWEEN '18410' AND '18433') AND GP.MES <" & Mid(ya,4,1) & "07 AND (m.IDentIfier = 'C') GROUP BY SUBSTRING(m.ProjCode, 1, 2)"%>
     <%mdbor7.Open mdbo7%>
     <tr Class="ProjGrup">
      <td>
     ...
     <%Do Until mdborl2.EOF%>
      ...
      <%mdbor2.Open mdbo2%>
      <%mdbo2u.CommandText="SELECT ROUND(SUM(ISNULL(GP.DEBET,0))/1000,0)-ROUND(SUM(ISNULL(GP.CREDIT,0))/1000,0) AS summi, SUBSTRING(m.ProjCode, 1, 2), SUBSTRING(m.ProjCode, 4, 2) AS be FROM dbo.Main m INNER JOIN dbo.glav_project GP ON m.OracleCode = GP.PROJECT WHERE  (ISNULL(DOCS_REGNR,0)=0 OR DOCS_REGNR<>'437700')   and m.Yearr='" & ya & "' AND SUBSTRING(m.ProjCode,1,5)='" & MId(mdborl2("PC"),1,5) & "' AND (SUBSTRING(GP.MES, 1, 1) = '" & Mid(ya,4,1) & "') AND (SUBSTRING(GP.MES, 2, 2) >= 04) AND (SUBSTRING(GP.MES, 2, 2) < 07) AND m.OracleCode='EJB206' AND (m.IDentIfier = 'C') GROUP BY SUBSTRING(m.ProjCode, 1, 2),SUBSTRING(m.ProjCode, 4, 2)"%>
      <%mdbor2u.Open mdbo2u%>     
      ...
      <%Do Until mdborl3.EOF%>
       ...
       <%Do Until mdbor.EOF%>
        ...
        <%If LEN(mdbor("PC"))>9 AND RIGHT(mdbor("PC"),2)="00" Then%>
         ...
         <%mdbor2u.Open mdbo2u%>
        <%Else%>
         <%mdbo2u.CommandText="SELECT ISNULL(ROUND(SUM(ISNULL(DEBET,0))/1000,0),0)-ISNULL(ROUND(SUM(ISNULL(CREDIT,0))/1000,0),0) AS summi, PROJECT FROM glav_project WHERE Project='" & mdbor("OracleCode") & "' AND  (ISNULL(DOCS_REGNR,0)=0 OR DOCS_REGNR<>'437700')  AND Project='EJB206' AND  LEFT(MES,1)=" & Mid(ya,4,1) & " AND RIGHT(MES,2) >=04 AND RIGHT(MES,2)<07 GROUP BY PROJECT ORDER BY PROJECT DESC "%>
         <%mdbor2u.Open mdbo2u%>
        <%End If%>
        <%If LEN(mdbor("PC"))>9 AND RIGHT(mdbor("PC"),2)="00" Then%>
        ...
        <%If LEN(mdbor("PC"))>9 AND RIGHT(mdbor("PC"),2)="00" Then%>
         ...
         <%mdbor7.Open mdbo7%>
        <%Else%>
         <%mdbo7.CommandText="SELECT ISNULL(ROUND(SUM(ISNULL(GP.DEBET,0))/1000,0),0)-ISNULL(ROUND(SUM(ISNULL(GP.CREDIT,0))/1000,0),0) AS summym FROM dbo.Main m INNER JOIN dbo.glav_project GP ON m.OracleCode = GP.PROJECT WHERE Project='" & mdbor("OracleCode") & "' AND m.Yearr='" & ya & "' AND (konto BETWEEN '18410' AND '18433') AND GP.MES <" & Mid(ya,4,1) & "07 AND (m.IDentIfier = 'C')"%>
         <%mdbor7.Open mdbo7%>
        <%End If%>
        <tr>
         <td>
    ...
    <%mdbor6a.Open mdbo6a%>
    <%mdbo7.CommandText="SELECT ISNULL(ROUND(SUM(ISNULL(GP.DEBET,0))/1000,0),0)-ISNULL(ROUND(SUM(ISNULL(GP.CREDIT,0))/1000,0),0) AS summym, Enterprise FROM dbo.Main m INNER JOIN dbo.glav_project GP ON m.OracleCode = GP.PROJECT WHERE m.Yearr='" & ya & "' AND (konto BETWEEN '18410' AND '18433') AND m.RenovBlock=0 AND GP.MES <" & Mid(ya,4,1) & "07 AND (m.IDentIfier = 'C') GROUP BY Enterprise ORDER BY Enterprise"%>
    <%mdbor7.Open mdbo7%>
